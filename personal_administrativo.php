...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
  ...
  if ($id) {
    ...
    if ($foto && $documento) {
      $stmt = $pdo->prepare(
        "UPDATE administrativo SET nombre = ?, cargo = ?, año_ingreso = ?, foto = ?, salario = ?, notas = ?, documento = ? WHERE id = ?"
      );
      $stmt->execute([$nombre, $cargo, $anio, $foto, $salario, $notas, $documento, $id]);
    } elseif ($foto) {
      $stmt = $pdo->prepare(
        "UPDATE administrativo SET nombre = ?, cargo = ?, año_ingreso = ?, foto = ?, salario = ?, notas = ? WHERE id = ?"
      );
      $stmt->execute([$nombre, $cargo, $anio, $foto, $salario, $notas, $id]);
    } elseif ($documento) {
      $stmt = $pdo->prepare(
        "UPDATE administrativo SET nombre = ?, cargo = ?, año_ingreso = ?, salario = ?, notas = ?, documento = ? WHERE id = ?"
      );
      $stmt->execute([$nombre, $cargo, $anio, $salario, $notas, $documento, $id]);
    } else {
      $stmt = $pdo->prepare(
        "UPDATE administrativo SET nombre = ?, cargo = ?, año_ingreso = ?, salario = ?, notas = ? WHERE id = ?"
      );
    ...
    $stmt = $pdo->prepare(
      "INSERT INTO administrativo (nombre, cargo, año_ingreso, foto, salario, notas, documento) VALUES (?, ?, ?, ?, ?, ?, ?)"
    );
